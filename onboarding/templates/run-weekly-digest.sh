...
cd "$WORKSPACE" && "$CLAUDE_BIN" \
  -p "Run the weekly digest skill. Read $WORKSPACE/skills/weekly-digest/SKILL.md and execute all steps fully. Your session name is ${SESSION_NAME} - use this exact string in the Slack notification resume command." \
  -n "${SESSION_NAME}" \
